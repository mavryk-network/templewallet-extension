...
export function operationsGroupToHistoryItem({ hash, operations }: OperationsGroup, address: string): UserHistoryItem {
  ...
  const type = deriveHistoryItemType(historyItemOperations);
...
function reduceOneTzktTransactionOperation(
  ...
  if (parameter == null) {
    ...
    return _buildReturn({ amount, source, contractAddress: operation.target.address, tokenTransfers });
  } else if (isTzktOperParam_Fa2(parameter)) {
    const tokenTransfers = buildTokenTransferItem(operation, 'fa2', address);
    console.log('Got to here in buildTokenTransferItem. Hash & Op:', operation.hash, operation, tokenTransfers);
...
function deriveHistoryItemType(items: IndividualHistoryItem[]): HistoryItemOpTypeEnum {
  const opType = HistoryItemOpTypeEnum.Other;
...
function buildTokenTransferItem(
  ...
  if (params === null) return null;
    ...
    const values = reduceParameterFa2Values(params, address);
    ...
    console.log('Got to here in the fa12 build token transfers', operation);
    return {
    ...
    console.log('Got to here in the tez build token transfers', operation);
    const source = operation.sender;
    const amount = String(operation.amount);
