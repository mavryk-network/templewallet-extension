...
export const rwasMetadataInitialState: SliceState = {
  records: new Map(),
  isLoading: false
};

/** Cannot use initial value during migrations - object is frozen & forbids mutations. */
export const rwasMetadataInitialStateClone = cloneDeep(rwasMetadataInitialState);

export const sanitizeRwasMetadataForDevTools = <S extends SliceState>(state: S): S => ({
  ...state,
